...
function wifidev.get_i18n(self)
	...
	if l.a then m = m .. "ac/n" end
	if l.b then m = m .. "bg/n" end
...
function wifinet.adminlink(self)
	if self.netid == "wifi0.network1" then
		return dsp.build_url("admin", "wifi", "wlan1")
	elseif self.netid == "wifi0.network2" then
		return dsp.build_url("admin", "wifi", "wlan1_guest")
	elseif self.netid == "wifi1.network1" then
		return dsp.build_url("admin", "wifi", "wlan2")	
	elseif self.netid == "wifi1.network2" then
		return dsp.build_url("admin", "wifi", "wlan2_guest")
	else	
...
function get_mac(self, iface)
	if type(iface) == "string" then
        return utl.exec('ifconfig ' .. iface .. " | grep HWaddr"):match("HWaddr ([%:%x]*)")
	else
		return "please give a valid interface string"
	end
end

function get_country(self)
	local uci = require("luci.model.uci").cursor()
	if '00' == uci:get("wireless","wifi0","country") then
		return "Please select the country"
	else
		return get_countrylist()[_get("wireless", "wifi0", "country")]
	end
end
...
function get_countrylist()
	local tbl = {}
	local domain = utl.exec("setconfig -g 4 | awk {'printf $1'}")
	if domain == "0" or domain == "2" then -- FCC / INT
		tbl["8"] = "Albania"
		tbl["12"] = "Algeria"
		tbl["51"] = "Armenia"
		tbl["32"] = "Argentina"
		...
		tbl["191"] = "Croatia"
		tbl["196"] = "Cyprus"
		tbl["203"] = "Czech Republic"
		tbl["208"] = "Denmark"
		tbl["214"] = "Dominican Republic"
		tbl["218"] = "Ecuador"
		tbl["818"] = "Egypt"
		tbl["222"] = "El Salvador"
		tbl["233"] = "Estonia"
		...
		tbl["788"] = "Tunisia"
		tbl["792"] = "Turkey"
		tbl["784"] = "United Arab Emirates"
		tbl["804"] = "Ukraine"
		tbl["826"] = "United Kingdom"
		tbl["829"] = "United Kingdom B"
		tbl["830"] = "United Kingdom C"
		tbl["858"] = "Uruguay "
		tbl["840"] = "USA"
		tbl["860"] = "Uzbekistan"
		tbl["862"] = "Venezuela "
