...
function index()
	entry({"admin", "system"}, alias("admin", "system", "system"), _("System"), 30)
	...
	entry({"admin", "system", "change_language"}, call("action_change_language"), nil, 100)
end
...
function action_change_language()
	...
	local current_uri = luci.http.formvalue("current_uri")
	if lang and current_uri then
	
		uci:set("luci", "main", "lang", lang)
		uci:save("luci")
		uci:commit("luci")
		uci:apply("luci")		
		--luci.util.exec("echo "..lang.."	"..current_uri.." > /dev/ttyS0")
		luci.http.redirect(luci.dispatcher.build_url(current_uri))
	end
end
