...
function m.on_before_apply(map)
	...
	local disabled24G = luci.http.formvalue("cbid.wireless.wifi0.disabled")
	if disabled24G == "1" then
	...
	luci.http.redirect(luci.dispatcher.build_url("admin/wifi/wlan1"))
end
...
----------Band Steering-----------------	
bandsteer = s:option(ListValue,"bandsteer","Band Steering")
bandsteer.widget = "radio"
bandsteer.orientation = "horizontal"
bandsteer:value("1",translate("Enable"))
bandsteer:value("0",translate("Disable"))
...
if hwtype == "qcawifi" then
	...
	htmode:value("HT20_40", translate("20MHz/40MHz"))
