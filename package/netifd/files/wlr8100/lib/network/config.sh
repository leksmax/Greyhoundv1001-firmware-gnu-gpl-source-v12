...
prepare_interface() {
	...
	ifconfig "$iface" 2>/dev/null >/dev/null && {
		...
		#exclude Interface UP with wireless interface named 'ath' 'wlan'
		case "$iface" in
		ath*|wlan*) ;;
		*)
	...
	case "$iftype" in
		bridge)
			...
			[ -x /usr/sbin/brctl ] && {
				...
				# set IP and MAC at br-guest
				if [ "$config" = "guest" ]; then
					local guestIP=$(ifconfig br-guest | grep Bcast | awk -F " " '{print $2}' | awk -F ":" '{print $2}')
					local guestMAC=$(ifconfig br-guest | grep HWaddr | awk -F " " '{print $5}')
					local ifaceMAC=$(ifconfig ${iface} | grep HWaddr | awk -F " " '{print $5}')
					local theother=

					config_get ipaddr "$config" ipaddr

					if [ "$guestIP" != "$ipaddr" ]; then
						echo "===== Set IP at br-guest =====" >> /dev/console
						ifconfig br-${config} $ipaddr
					fi

					if [ "$guestMAC" != "$ifaceMAC" ]; then
						case "$iface" in
						ath27)
							ifconfig ath57 2>/dev/null >/dev/null && theother=$(ifconfig ath57 | grep HWaddr | awk -F " " '{print $5}')
						;;
						ath57)
							ifconfig ath27 2>/dev/null >/dev/null && theother=$(ifconfig ath27 | grep HWaddr | awk -F " " '{print $5}')
						;;
						esac

						if [ "$guestMAC" != "$theother" ]; then
							echo "===== Set MAC at br-guest =====" >> /dev/console
							ifconfig br-${config} hw ether $ifaceMAC
						fi
					fi
				fi
				#exclude Interface UP with wireless interface named 'ath' 'wlan'
				case "$iface" in
				ath08|ath24|ath18|ath54) #nawds 
