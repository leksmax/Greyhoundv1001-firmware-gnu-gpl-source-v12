    ...
    switch (inputMsg[0])
    {
        ...
        case subSystemDispatch:                 
			...
			if(inputMsg[80]==100){
				...
				inputMsg[16]=0x3;
				inputMsg[32]=0x1;
				if ( ( myStatus = processTLVPacket(inputMsg,cmdLen-19-ESCAPEcnt-16, respdata, &nrespdata) ) > 0 )
			    {
					printf( " processing dummy TLVPacket(,,,) returns %d\n", myStatus );
					//adding return value
					if (flashSupport==0){
